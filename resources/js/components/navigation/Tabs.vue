...
const tabs = ref({
    'tribe': ['', 'history', 'conquers', 'stats', 'members', 'changes'],
    'player': ['', 'history', 'conquers', 'stats', 'villages', 'changes'],
    'village': ['', 'history', 'conquers']
});
const map = ref({
...
<ul class="nav nav-tabs mt-4">
    <li v-for="tab in tabs[props.endpoint]" :key="tab" :class="tab==='conquers' ? 'nav-item dropdown' : 'nav-item'">
        <a v-if="tab==='conquers' && props.active === tab"
        ...
        <div v-if="tab==='conquers'" class="dropdown-menu">
